...
from core.ui import HeaderUI, ProgressBar, Button, Card, PRIMARY, SECONDARY, ACCENT, LIGHT_TEXT, DARK_BG, WHITE, BLACK, render_text_wrapped
...
class LessonScene:
    ...
    def _draw_ui(self):
        ...
        # Prompt (main question) - with text wrapping to prevent overflow
        prompt_max_width = card_rect.width - 80
        prompt_lines = render_text_wrapped(self.prompt_font, card["prompt"], (20, 20, 20), prompt_max_width)
        prompt_y = card_rect.y + 90
        for line in prompt_lines[:2]:  # Limit to 2 lines
            self.screen.blit(line, (card_rect.x + 40, prompt_y))
            prompt_y += line.get_height() + 5
        ...
        for i, answer in enumerate(targets[:3]):
            ...
            # Scale font down if answer is too long
            answer_text = f"✓ {answer}"
            answer_max_width = answer_rect.width - 40
            ans_txt = self.target_font.render(answer_text, True, SECONDARY)
            
            if ans_txt.get_width() > answer_max_width:
                current_size = 32
                while current_size > 16 and ans_txt.get_width() > answer_max_width:
                    current_size -= 2
                    scaled_font = pygame.font.Font(FONT_PATH, current_size)
                    ans_txt = scaled_font.render(answer_text, True, SECONDARY)
    ...
    def _draw_feedback(self):
        ...
        # Scale down font if spoken text is too long
        max_spoken_width = fb_rect.width - 80
        spoken_text = f"คุณพูด: '{text}'"
        spoken_txt = self.small_font.render(spoken_text, True, text_color)
        
        # Scale font down if needed
        if spoken_txt.get_width() > max_spoken_width:
            current_size = 24
            while current_size > 14 and spoken_txt.get_width() > max_spoken_width:
                current_size -= 2
                scaled_font = pygame.font.Font(FONT_PATH, current_size)
                spoken_txt = scaled_font.render(spoken_text, True, text_color)
